...
Flatbush.prototype.finish = function finish () {
    ...
    for (var i = 0; i < this.numItems; i++) {
        var pos = 4 * i;
        var minX = this.boxes[pos++];
        var minY = this.boxes[pos++];
        var maxX = this.boxes[pos++];
        var maxY = this.boxes[pos++];
    ...
    for (var i$1 = 0, pos$1 = 0; i$1 < this.levelBounds.length - 1; i$1++) {
        var end = this.levelBounds[i$1];
        ...
        while (pos$1 < end) {
            ...
            for (var ii = 0; ii < this.nodeSize && pos$1 < end; ii++) {
                nodeMinX = Math.min(nodeMinX, this.boxes[pos$1++]);
                nodeMinY = Math.min(nodeMinY, this.boxes[pos$1++]);
                nodeMaxX = Math.max(nodeMaxX, this.boxes[pos$1++]);
                nodeMaxY = Math.max(nodeMaxY, this.boxes[pos$1++]);
            }
...
Flatbush.prototype.search = function search (minX, minY, maxX, maxY, filterFn) {
    ...
    var nodeIndex = this.boxes.length - 4;
    var queue = [];
    var results = [];
    while (nodeIndex !== undefined) {
        // find the end index of the node
        var end = Math.min(nodeIndex + this.nodeSize * 4, upperBound(nodeIndex, this.levelBounds));
        // search through child nodes
        for (var pos = nodeIndex; pos < end; pos += 4) {
            var index = this.indices[pos >> 2] | 0;
            // check if node bbox intersects with query bbox
            if (maxX < this.boxes[pos])
                { continue; } // maxX < nodeMinX
            if (maxY < this.boxes[pos + 1])
                { continue; } // maxY < nodeMinY
            if (minX > this.boxes[pos + 2])
                { continue; } // minX > nodeMaxX
            if (minY > this.boxes[pos + 3])
...
Flatbush.prototype.neighbors = function neighbors (x, y, maxResults, maxDistance, filterFn) {
    ...
    while (nodeIndex !== undefined) {
        ...
        for (var pos = nodeIndex; pos < end; pos += 4) {
            var index = this.indices[pos >> 2] | 0;
            var dx = axisDist(x, this.boxes[pos], this.boxes[pos + 2]);
            var dy = axisDist(y, this.boxes[pos + 1], this.boxes[pos + 3]);
