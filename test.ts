...
import Flatbush from './src/index';
...
function createSmallIndex(numItems: number, nodeSize: number): Flatbush {
...
it('indexes a bunch of rectangles', () => {
  const index = createIndex();

  const len = index.boxes.length;
  assert.strictEqual(index.boxes.length + index.indices.length, 540);
  assert.deepStrictEqual(Array.from(index.boxes.subarray(len - 4, len)), [0, 1, 96, 95]);
  assert.deepStrictEqual(index.indices[len / 4 - 1], 400);
});

it('skips sorting less than nodeSize number of rectangles', () => {
  const numItems = 14;
  const nodeSize = 16;
  ...
  assert.deepStrictEqual(Array.from(index.indices), expectedIndices);
  assert.strictEqual(len, (numItems + 1) * 4);
  assert.deepStrictEqual(Array.from(index.boxes.subarray(len - 4, len)), [rootXMin, rootYMin, rootXMax, rootYMax]);
});

it('performs bbox search', () => {
  const index = createIndex();
  ...
  assert.deepStrictEqual(results.sort(compare), [57, 59, 58, 59, 48, 53, 52, 56, 40, 42, 43, 43, 43, 41, 47, 43].sort(compare));
});

it('reconstructs an index from array buffer', () => {
  const index = createIndex();
  const index2 = Flatbush.from(index.data);

  assert.deepStrictEqual(index, index2);
});

it('throws an error if added less items than the index size', () => {
  assert.throws(() => {
    const index = new Flatbush(data.length / 4);
    index.finish();
  });
...
it('throws an error if searching before indexing', () => {
  assert.throws(() => {
    const index = new Flatbush(data.length / 4);
...
it('does not freeze on numItems = 0', () => {
  assert.throws(() => {
    new Flatbush(0); // eslint-disable-line
  });
});

it('performs a k-nearest-neighbors query', () => {
  const index = createIndex();
  const ids = index.neighbors(50, 50, 3);
  assert.deepStrictEqual(ids.sort(compare), [31, 6, 75].sort(compare));
});

it('k-nearest-neighbors query accepts maxDistance', () => {
  const index = createIndex();
  const ids = index.neighbors(50, 50, Infinity, 12);
  assert.deepStrictEqual(ids.sort(compare), [6, 29, 31, 75, 85].sort(compare));
});

it('k-nearest-neighbors query accepts filterFn', () => {
  const index = createIndex();
  const ids = index.neighbors(50, 50, 6, Infinity, (i) => i % 2 === 0);
  assert.deepStrictEqual(ids.sort(compare), [6, 16, 18, 24, 54, 80].sort(compare));
});

it('returns index of newly-added rectangle', () => {
  const count = 5;
  ...
  assert.deepStrictEqual(ids, expectedSequence);
});
