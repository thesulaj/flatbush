...
// @ts-ignore
import RBush from 'rbush'; // eslint-disable-line
// @ts-ignore
import rbushKNN from 'rbush-knn';  // eslint-disable-line
...
function addRandomBox(arr: number[], boxSize: number) {
...
const boxes100: number[] = [];
const boxes10: number[] = [];
const boxes1: number[] = [];
for (let i = 0; i < K; i++) {
...
function benchSearch(boxes: number[], name: string, warmup?: boolean) {
...
function benchNeighbors(k: number, M: number, warmup?: boolean) {
...
function benchSearchRBush(boxes: number[], name: string, warmup?: boolean) {
  const boxes2 = [];
  for (let i = 0; i < boxes.length; i += 4) {
    boxes2.push({
      minX: boxes[i],
...
function benchNeighborsRBush(k: number, M: number, warmup?: boolean) {
  const id = `${k} searches of ${M} neighbors`;
  if (!warmup) console.time(id);
  for (let i = 0; i < k; i++) {
